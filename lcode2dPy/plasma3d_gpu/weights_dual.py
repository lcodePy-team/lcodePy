...
def deposit(
    ...
    x_offt, y_offt, px, py, pz, q, m, const_arrays
):
    """
    Interpolate coarse plasma into fine plasma and deposit it on the
    charge density and current grids.
    ...
    """
    virtplasma_smallness_factor = 1 / (plasma_coarseness *
                                       plasma_fineness)**2
    ...
    cfg = int(cp.ceil(const_arrays.fine_grid.size**2 / WARP_SIZE)), WARP_SIZE
...
def initial_deposition(
    ...
    ro_electrons_initial, _, _, _ = deposit(
        grid_steps, grid_step_size, plasma_coarseness, plasma_fineness,
        x_offt, y_offt, px, py, pz, q, m, virt_params
    )
